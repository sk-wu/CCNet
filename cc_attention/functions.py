...
class CrissCrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out_H = torch.bmm(proj_value_H, att_H.permute(0, 2, 1)).view(m_batchsize,width,-1,height).permute(0,2,3,1) # (B*W,C,H)*(B*W,H,H)->(B*W,C,H)->(B,C,H,W)
        out_W = torch.bmm(proj_value_W, att_W.permute(0, 2, 1)).view(m_batchsize,height,-1,width).permute(0,2,1,3) # (B,W,C,W)*(B*H,W,W)->(B*W,C,H)->(B,C,H,W)
